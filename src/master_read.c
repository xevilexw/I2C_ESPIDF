...
void app_main() {
    ...
    /* Write slave addr */
    i2c_master_write_byte(i2c_cmd_handle, (I2C_SLAVE_ADDR << 1) | I2C_MASTER_WRITE, true);

    /* Read data from this address */
    i2c_master_read(i2c_cmd_handle, received_data, sizeof(received_data), true);
    i2c_master_stop(i2c_cmd_handle);

    /* Reading with error logging... */
